...
class CrossAttentionFusion(nn.Module):
    ...
    def forward(self, x1, x2):
        ...
        # Manually replicate cross-attention using the combined in_proj_weight matrix

        # 1. Unpack the combined weight and bias from the nn.MultiheadAttention layer
        W_in = self.attn.in_proj_weight
        B_in = self.attn.in_proj_bias

        # 2. Split the packed weights and biases into Q, K, and V components
        W_q, W_k, W_v = W_in.chunk(3)
        if B_in is not None:
            B_q, B_k, B_v = B_in.chunk(3)
        else:
            B_q, B_k, B_v = None, None, None

        # 3. Project Q from the first input (x1) and K, V from the second input (x2)
        ...
        q = q.contiguous().view(batch_size, seq_len_q, self.attn.num_heads, self.attn.head_dim).transpose(1, 2)
        k = k.contiguous().view(batch_size, seq_len_kv, self.attn.num_heads, self.attn.head_dim).transpose(1, 2)
        v = v.contiguous().view(batch_size, seq_len_kv, self.attn.num_heads, self.attn.head_dim).transpose(1, 2)

        # 5. Calculate attention scores
        attention_scores = torch.matmul(q, k.transpose(-2, -1)) / math.sqrt(self.attn.head_dim)

        # 6. Apply stable softmax (This was the original goal)
        attn_weights = F.softmax(attention_scores, dim=-1, dtype=torch.float32)
        attn_weights_for_output = self.dropout(attn_weights)

        # 7. Apply attention to values and project the final output
        attn_output = torch.matmul(attn_weights_for_output, v).transpose(1, 2).contiguous().view(batch_size, seq_len_q,
                                                                                                 embed_dim)
        fused_out = self.attn.out_proj(attn_output)
...
class HGCNLayer(nn.Module):
    ...
    def forward(self, x, bone_features, dynamic_adj=None):
        ...
        if self.use_hybrid_graph:
            ...
            base_adj = base_adj + self.static_adj
...
class TemporalAttentionBlock_V4(nn.Module):
    ...
    def forward(self, x, rotary_emb, key_padding_mask=None):
        ...
        # --- START OF MODIFICATION TO FIX NaN ISSUE ---
        # The original `self.attn(...)` call can be unstable with autocast.
        # We manually replicate the attention calculation to ensure softmax is done in float32.

        # 1. Manually project q, k, v using the weights from the nn.MultiheadAttention layer
        q, k, v = F.linear(q_rot, self.attn.in_proj_weight, self.attn.in_proj_bias).chunk(3, dim=-1)

        # 2. Reshape for multi-head attention
        batch_size, seq_len, embed_dim = q.shape
        q = q.contiguous().view(batch_size, seq_len, self.attn.num_heads, self.attn.head_dim).transpose(1, 2)
        k = k.contiguous().view(batch_size, seq_len, self.attn.num_heads, self.attn.head_dim).transpose(1, 2)
        v = v.contiguous().view(batch_size, seq_len, self.attn.num_heads, self.attn.head_dim).transpose(1, 2)
        ...
        # 4. Apply stable softmax (THE KEY FIX)
        # Forcing float32 prevents overflow and the resulting NaN values.
        attn_weights = F.softmax(attention_scores, dim=-1, dtype=torch.float32)
        attn_weights_for_output = self.dropout(attn_weights)

        # 5. Apply attention to values and project the final output
        attn_output = torch.matmul(attn_weights_for_output, v).transpose(1, 2).contiguous().view(batch_size, seq_len,
                                                                                                 embed_dim)
        attn_out = self.attn.out_proj(attn_output)
        ...
        # Return the attention weights for potential analysis later
