...
def train_one_experiment(experiment_name, config, datasets, device):
    ...
    # Instantiate loss functions ONCE before the loop for efficiency and correctness
    ...
    for epoch in range(config['epochs']):
        ...
        for data in pbar:
            ...
            if not torch.isfinite(total_loss):
                logging.warning(f"Epoch {epoch + 1}: Invalid loss detected (NaN or Inf). Skipping batch.")
                continue
