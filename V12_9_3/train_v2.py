...
from utils_v2 import (load_and_preprocess_sequential_data, log_to_file,
                      ...
                      validate_batch_data, check_model_initialization)
...
def train_one_experiment(experiment_name, config, datasets, device):
    ...
    if not check_model_initialization(model):
        raise RuntimeError("Model initialization failed, contains NaN/Inf parameters.")
    ...
    if config.get('use_knowledge_distillation', False):
        pass  # Distillation logic unchanged
    ...
    anatomical_criterion = AnatomicalLoss(config={'device': device})
    ...
    for epoch in range(config['epochs']):
        ...
        for data in pbar:
            ...
            with autocast(device_type=device.type, enabled=True):
                try:
                    ...
                    if not torch.isfinite(outputs['logits']).all():
                        logging.warning("NaN/Inf detected in model logits, skipping batch.")
                        continue

                    class_loss = mixup_criterion(criterion, outputs['logits'], y_a, y_b, lam)
                    total_loss = class_loss
                    if torch.isfinite(class_loss): epoch_losses['class_loss'] += class_loss.item()

                    if config.get('anatomical_loss_weight', 0.0) > 0:
                        anatomical_loss_dict = anatomical_criterion(outputs['anatomical_prediction'])
                        anatomical_loss = config['anatomical_loss_weight'] * anatomical_loss_dict['total']
                        if torch.isfinite(anatomical_loss):
                    ...
                    if model_params.get('use_dynamic_graph', False) and sparsity_weight > 0:
                        ...
                        if spatial_graphs:
                            ...
                            if torch.isfinite(sparsity_loss):
                                total_loss += sparsity_loss
                                epoch_losses['sparsity_loss'] += sparsity_loss.item()

                except RuntimeError as e:
                    logging.error(f"Runtime error during forward pass: {e}. Skipping batch.")
                    continue

            if not torch.isfinite(total_loss):
                logging.warning(f"Final total_loss is NaN/Inf. Skipping batch.")
                continue
            ...
            # This is optional but good practice to see the true gradient norm.
            # scaler.step will do this internally anyway.
            scaler.unscale_(optimizer)
            torch.nn.utils.clip_grad_norm_(model.parameters(), max_norm=1.0)

            # 3. scaler.step() automatically checks for NaNs/Infs and skips the update.
            # This is the robust way to handle gradient issues with AMP.
        ...
        if math.isnan(val_loss):
            ...
            logging.warning(f"Epoch {epoch + 1}: Validation loss is NaN. Metrics will be zero.")
        else:
        ...
        avg_train_loss = epoch_losses['total'] / batches_processed if batches_processed > 0 else 0.0
