...
def validate_batch_data(batch_data):
    """验证批次数据的有效性，防止NaN或极端值进入模型。"""

    for i, tensor in enumerate(batch_data):
        if tensor is None:
            continue

        # 检查NaN和Inf
        if not torch.isfinite(tensor).all():
            logging.warning(f"Invalid values (NaN/Inf) in batch tensor {i}, skipping batch.")
            return False

        # 检查数据范围（可根据实际情况调整）
        if tensor.dtype.is_floating_point:
            abs_max = tensor.abs().max()
            if abs_max > 1e4:  # 检查是否存在极端值
                logging.warning(f"Extreme values in batch tensor {i}: max_abs={abs_max}, skipping batch.")
                return False
...
def check_model_initialization(model):
    """检查模型初始化的合理性，确保没有NaN权重。"""

    for name, param in model.named_parameters():
        if param.requires_grad:
            # 检查参数是否包含NaN
            if not torch.isfinite(param).all():
                logging.error(f"FATAL: Parameter {name} contains NaN/Inf after initialization.")
                return False
    logging.info("Model initialization check passed. All parameters are finite.")
    return True