...
class CrossAttentionFusion(nn.Module):
    ...
    def forward(self, x1, x2):
        fused_out, _ = self.attn(query=self.query_norm(x1), key=self.kv_norm(x2), value=self.kv_norm(x2));
        return x1 + self.dropout(fused_out)


# NEW: Add the GraphLearner module for Dynamic GNN
...
class HGCNLayer(nn.Module):
    ...
    # MODIFIED: Update forward pass to accept and use the dynamic adjacency matrix
    def forward(self, x, bone_features, dynamic_adj=None):
        ...
        # Calculate the self-attention adjacency matrix
        self_attn_adj = F.softmax((q @ k.transpose(-2, -1)) / (self.head_dim ** 0.5), dim=-1)

        # If a dynamic graph is provided, use it to weigh the self-attention matrix
        if dynamic_adj is not None:
            # unsqueeze(1) is for broadcasting across the multi-head dimension
            final_adj = self_attn_adj * dynamic_adj.unsqueeze(1)
        else:
            final_adj = self_attn_adj
...
class GNNBlock(nn.Module):
    # MODIFIED: Add a 'use_dynamic_graph' flag and instantiate GraphLearner
    ...
    def forward(self, x, bone_features=None):
        ...
        # MODIFIED: Calculate the dynamic graph if the feature is enabled
        if self.use_dynamic_graph:
            h_norm = self.layers[0][0](x)  # Use normalized features to learn the graph
            dynamic_adj = self.graph_learner(h_norm)

        for norm, layer in self.layers:
            h = norm(x);
            # MODIFIED: Pass the dynamic_adj to the HGCNLayer
...
class SpatioTemporalGraphBlock(nn.Module):
    ...
    def __init__(self, embed_dim, num_heads, dropout, num_gnn_layers=2, gnn_type='HGCN', gnn_residual=True, **kwargs):
        super().__init__()
        # MODIFIED: Pass all kwargs to GNNBlock to handle the new 'use_dynamic_graph' flag
...
class TemporalAttentionBlock_V4(nn.Module):
    ...
    def forward(self, x, rotary_emb, key_padding_mask=None):
        ...
        attn_out, attn_weights = self.attn(q_rot, k_rot, h, key_padding_mask=key_padding_mask);
...
class DHSGNet_V4(nn.Module):
    ...
    def forward(self, x, skeletal_features=None, key_padding_mask=None, labels=None) -> Dict[
        ...
        policy_logits_list, spatial_attn_weights, temporal_attn_weights = [], None, None
        ...
        for i, (block, drop_path) in enumerate(self.blocks):
            ...
            if isinstance(block, SpatioTemporalGraphBlock):
                ...
                processed, current_spatial_attn = block(x_spatial_in, bone_features=expanded_bone_features)
                processed_reshaped = processed.view(B, T, N, embed_dim)
                x_embedded = x_embedded + drop_path(processed_reshaped)
                if current_spatial_attn: spatial_attn_weights = current_spatial_attn
            else:
                ...
                final_cls_token, temporal_attn_weights = final_cls_token + processed[:, :1], current_temporal_attn
        ...
        return {
            ...
            'policy_logits': policy_logits_list, 'temporal_attention': temporal_attn_weights,
