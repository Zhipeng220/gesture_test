...
from torch.nn import CrossEntropyLoss  # CORRECTED: Imported from torch.nn
...
# CORRECTED: Removed CrossEntropyLoss from this custom import
from losses_v2 import FocalLoss, AnatomicalLoss, InfoNCELoss, DistillationLoss
...
def create_optimizer_with_warmup(model, config):
    if config.get('optimizer_type', 'adamw_custom') == 'adamw_custom':
        ...
        # CORRECTED: Ensure T_max is always at least 1 to prevent division by zero.
...
def train_one_experiment(experiment_name, config, datasets, device):
    ...
    if 'pretrained_backbone_path' in model_params:
        ...
        # Load state_dict with strict=False to ignore classifier weights if they don't match
    ...
    criterion = base_crit  # Start with the base criterion
    if config.get('use_knowledge_distillation', False):
        ...
        # Overwrite the criterion with the distillation version for training
        criterion = DistillationLoss(
            base_criterion=base_crit,
            teacher_model=teacher_model,
            alpha=config.get('distillation_alpha', 0.3),
            temperature=config.get('distillation_temp', 2.0)
        )
    ...
    for epoch in range(config['epochs']):
        ...
        for data in train_loader:
            ...
            with autocast(device_type=device.type, enabled=scaler.is_enabled()):
                ...
                if config.get('anatomical_loss_weight', 0.0) > 0:
                    total_loss += config['anatomical_loss_weight'] * \
                                  AnatomicalLoss(config={'device': device})(outputs['anatomical_prediction'])['total']
        ...
        with torch.no_grad():
            for batch in val_loader:
                ...
                # CORRECTED: Always use the simple base criterion for validation loss
        ...
        history.update({'val_loss': [val_loss / len(val_loader)], 'val_acc': [val_acc], 'val_f1': [val_f1],
                        'train_loss': [epoch_losses['total'] / len(train_loader)]})
        ...
        if early_stopping.early_stop: logging.info("Early stopping triggered."); break
...
def pretrain_one_experiment(experiment_name, config, datasets, device):
    ...
    # NEW: Enable GradScaler for mixed-precision training
    scaler = GradScaler(enabled=device.type == 'cuda')
    ...
    for epoch in range(config['epochs']):
        ...
        for data_batch in tqdm(pretrain_loader, desc=f"Pre-training Epoch {epoch + 1}"):
            ...
            # NEW: Use the scaler for the backward pass and optimizer step
