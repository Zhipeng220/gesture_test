...
def get_experiments():
    ...
    experiments = {
        # --- Baselines from V12_7 for comparison ---
        ...
        # --- Initial Dynamic GNN Experiment ---
        ...
        # --- Tuning Dynamic GNN (top_k) ---
        "20_Exp_V17_DynamicGNN_Tune_K": {
            'batch_size': 8, 'epochs': 200, 'optimizer_type': 'adamw_custom', 'learning_rate': 5e-5,
            'warmup_epochs': 10, 'classifier_lr_mult': 2.0, 'scheduler_type': 'cosine',
            'loss_function': 'focal', 'focal_gamma': 1.8, 'label_smoothing': 0.05, 'weight_decay': 1e-2,
        ...
        # --- Tuning Dynamic GNN (Learning Rate) ---
        "21_Exp_V17_DynamicGNN_Tune_LR": {
            'batch_size': 8, 'epochs': 200, 'optimizer_type': 'adamw_custom',
            'learning_rate': 3e-5,
            'warmup_epochs': 10, 'classifier_lr_mult': 2.0, 'scheduler_type': 'cosine',
            'loss_function': 'focal', 'focal_gamma': 1.8, 'label_smoothing': 0.05, 'weight_decay': 1e-2,
            'anatomical_loss_weight': 0.2, 'mixup_alpha': 0.4, 'early_stopping_patience': 40,
        ...
        # --- Test synergy between Dynamic GNN and V18 architecture ---
        "22_Exp_V18_With_DynamicGNN": {
            'batch_size': 8, 'epochs': 200, 'optimizer_type': 'adamw_custom', 'learning_rate': 5e-5,
            'warmup_epochs': 10, 'classifier_lr_mult': 2.0, 'scheduler_type': 'cosine',
            'loss_function': 'focal', 'focal_gamma': 1.8, 'label_smoothing': 0.05, 'weight_decay': 1e-2,
            'anatomical_loss_weight': 0.2, 'mixup_alpha': 0.4, 'early_stopping_patience': 40,
        ...
        # --- NEW (Phase 2): Hybrid Dynamic GNN with Sparsity Loss ---
        "23_Exp_V17_HybridDynamicGNN": {
            ...
            'anatomical_loss_weight': 0.2, 'mixup_alpha': 0.4,
            'early_stopping_patience': 40,
            # NEW: Add sparsity loss weight
            'sparsity_loss_weight': 0.1,
            'model_params': {
                'gnn_residual': True, 'dropout': 0.25, 'attention_dropout': 0.1, 'stochastic_depth_rate': 0.15,
                'use_arcface': False,
                'use_dynamic_graph': True,  # Must be true for these features
                'top_k': 5,
                # NEW: Enable the hybrid graph feature
